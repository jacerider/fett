...
Drupal.behaviors.fettMobileMenu = {
  ...
  trailBuild: function() {
    ...
    var links = [];
    var active = $('li.active-trail.children > a, li.active-trail.children > span', this.$menu).each(function(){
  ...
  levelRevert: function(level) {
    ...
    self.$menu.attr('data-depth', level);
    setTimeout(function(){
      var current = $('ul[data-level="' + level + '"] li.active-trail').removeClass('active-trail');
      self.animating = false;
    }, self.transitionDelay);
  }
};
